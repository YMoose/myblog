...
## 6. 形式语言与自动机理论
### 6.1. the chomsky hierarchy 乔姆斯基谱系
可以将自动机看成一个判断机，接受输入根据内部的逻辑判断是否接受
语言学家诺姆·乔姆斯基（Noam Chomsky）基于自动机理论提出了生成语法理论，并结合这一理论对接收器进行了有益的分类。接收器（acceptor）是一个没有输出的自动机，在特殊意义上，它按照预定规则接受机器字母表上的单词。
...
### 6.1.1. type3 正则语言 - 有限状态机（FSM Finite-state machine）：存储有限的状态
#### 6.1.1.1. 正则文法表示
正则文法的产生式满足以下形式
```
A->Bx
...
##### 6.1.1.2.2. 封闭性（closure property）
...
5. 正则的逆同态也是正则的
##### 6.1.1.2.3. 判定性质（decision property）
正则语言具有**判定性质（decision property）**，考虑一些基本问题：
1. 所描述语言是否为空，以一个有穷自动机来看，即从初始状态开始接受状态是否可达。
2. 具体的串*w*是否属于所描述语言，以一个有穷自动机来看，可以直接用*w*在自动机上模拟
3. 语言的等价性，将DFA最小化后比对
### 6.1.2. type2 上下文无关语言 - 下推自动机（PDA Pushdown automata）：存储有限的状态，下推存储器
#### 6.1.2.1. 上下文无关文法表示
```
A->a
A属于非终结符号
...
A->xA是右递归文法
#### 6.1.2.2. 上下文无关语言性质
### 6.1.3. type1 上下文有关语言 - 线性有界自动机（LBA Linear-bounded automata）：存储有限的状态，下推存储器，输入输出带本身
### 6.1.4. type0 无限制语言 - 图灵机（Turing machine）：
