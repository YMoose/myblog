...
```
### 1.3. 缓存优化
为了填补cpu速度与内存存取速度的鸿沟，cpu提出了多级存储架构。在多数现代处理器架构中，每个逻辑cpu有自己的本地cache。同时为了提升速度，提出了Relaxed/Weak Memory ModelWeakly ordered memory types
...
5. [Hardware Memory Models](https://research.swtch.com/hwmm)
