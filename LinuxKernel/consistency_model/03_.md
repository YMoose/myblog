...
```
### 2.1. 线程间同步操作
同步操作的目的是防止数据操作的顺序发生问题，其本质上也是一种特殊的共享数据操作（**信号量（semaphore）**就是这种特殊数据操作所操作的数据），其实现必须是原子的，不会被打断的，否则其自身的执行顺序被打断，就更无法发挥其对临界区数据操作顺序的保护作用了。为了达成目的，无法通过现有的常规的数据操作的硬件指令实现，需要重新在硬件层面实现同步操作。信号量是最基本的同步原语，如果有更复杂的同步操作原语需求，可进一步丰富其语义。
#### 2.1.1. 原子指令
