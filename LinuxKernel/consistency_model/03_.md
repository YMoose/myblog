...
- cache coherence：缓存一致性问题是支持内存一致性的组成部分之一。其目的是让硬件cache在对共享内存的读写操作没有任何影响，就和没有cache是一样的。问题的主要由硬件工程师解决，不在本文章中做重点讨论。
...
```
### 1.3. 缓存优化
上面提到的两个优化的导致的问题都是由于指令执行的顺序乱序导致的，还未涉及内存一致性问题。其解决方案也都是在[内存顺序一致性模型](#参考5)的前提下有效的。
![sequential consistency model](../pic/sequential_consistency_mem_model.png)
内存顺序一致性模型是最理想情况下的模型（MIPS R10000使用的此类内存模型），其假设所有的处理器直接连接到一块共享的每次只允许一个处理器读/写的内存上且处理器和共享内存间没有cache。
- 每次任意一个处理器对共享内存上的读，直接从共享内存读。
- 每次任意一个处理器对共享内存上的写，直接写到共享内存。
然而！
现代处理器为了填补cpu速度与内存存取速度的鸿沟，cpu提出了多级存储架构。在多数现代处理器架构中，每个逻辑cpu有自己的本地写缓存（local write buffer），本地写缓存对共享内存的写回策略使得宽松内存模型（Relaxed/Weak Memory Model）产生。
下面介绍几个常见的宽松内存模型
#### 1.3.1. x86 Total Store Order(x86-TSO)
![x86 TSO](../pic/x86_tso_model.png)
x86下的内存模型比起顺序一致性模型多了一个local write queue（a write back cache）。
- 每次任意一个处理器对共享内存上的读，先从local write queue查看是否存在，存在直接读local write queue，不存在从共享内存读。
- 每次任意一个处理器对共享内存上的写，先写到local write queue，再一起写到共享内存。
``` C

```

##### 1.3.1.1. 指令优化导致的并发问题解决方法
#### 1.3.2. ARM/POWER Relaxed Momory Moder 
...
## 最终解决方法————锁
## 2. 内存一致性模型
上述提到了一些内存模型，内存模型是一个规范，指明了使用共享内存执行的多线程程序所被允许的行为，目的是为了精确定义
（a）编程者能够期望什么行为 
 (b)系统实现者可以使用哪些优化
### 2.1. 顺序一致性模型（SC）
最直观的内存模型就是顺序一致性（sequential consistency）
