...
无数据竞争程序的中有两种操作
1. synchronization（同步操作）：包括lock acquire 和 release
2. data（数据操作）
```
实现无数据竞争程序
  ...
  |-一个SC execution是DRF
    |-需要其没有数据操作的竞争
      |-一个数据操作没有竞争
        |-要么数据操作没有冲突
          |-一个数据操作没有冲突
            |-一个数据操作有冲突是指数据操作来自不同线程，访问相同的内存位置且其中至少有一个是store操作
...
9. [A Primer On Memory Consistency And Cache Coherence（内存一致性文章原文）](./APrimerOnMemoryConsistencyAndCacheCoherence.pdf)
